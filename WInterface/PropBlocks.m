...
- (void)readLine:(NSString *)line {
    if (inIFace) {
        ...
        else if ([line hasPrefix:@"- (void)dealloc"]) {
            inDealloc = YES;
        }
        else if ([line hasPrefix:@"- (void)die"]) {
            inDie = YES;
        }
        else if ([line hasPrefix:@"- (void)getPropertiesUsingContext:(NSMutableDictionary*)"]) {
            inInit = YES;
        }
        else {
            [self.ifaceStr appendFormat:@"%@\n",line];
        }
    }
    else if (inImpl) {
        if ([line hasPrefix:@"@end"]) {

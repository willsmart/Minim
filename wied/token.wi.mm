...
// !!!: Implementations: j
    ...
    - (void)_startObjectOfClassJsonParser {
        MSGSTART("JsonParser:-(void)_startObjectOfClassJsonParser")
        ...
        /*i0*/}
    + (NSObject *)_unpack:(NSArray *)tokens from:(Unsigned)from to:(Unsigned)to used:(Unsigned *)pused {
        MSGSTART("JsonParser:+(NSObject*)_unpack:(NSArray*)tokens from:(Unsigned)from to:(Unsigned)to used:(Unsigned*)pused")

        if (to < from) {
            *pused = 0; return [NSNull null];
        }
        WReaderToken *t0 = [tokens objectAtIndex:from];
        if (t0.type == 'n') {
            *pused = 1; return @(t0.str.doubleValue);
        }
        if (t0.type == 's') {
        ...
        if ([t0.str isEqualToString:@"true"]) {
            *pused = 1; return @(YES);
        }
        if ([t0.str isEqualToString:@"false"]) {
            *pused = 1; return @(NO);
        }
        if ([t0.str isEqualToString:@"null"]) {
            *pused = 1; return [NSNull null];
        }
        if ([t0.str isEqualToString:@"undefined"]) {
            *pused = 1; return [NSNull null];
        }
...
/*i999*/}
    - (NSString *)description {
        MSGSTART("JsonParser:-(NSString*)description")
    ...
    - (void)die {
        MSGSTART("JsonParser:-(void)die")

        /*i900*/}
    - (NSMutableString *)objectName {
        MSGSTART("JsonParser:-(NSMutableString*)objectName")
    ...
    + (NSObject *)unpackString:(NSString *)s {
        MSGSTART("JsonParser:+(NSObject*)unpackString:(NSString*)s")
